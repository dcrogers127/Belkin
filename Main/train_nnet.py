...
net = buildNetwork(TrainAppSub[TrainAppSub.columns[vars_to_include]].shape[1], \
    TrainAppSub[TrainAppSub.columns[vars_to_include]].shape[1]-100, \
    TrainAppSub[TrainAppSub.columns[vars_to_include]].shape[1]-1000, \
    TrainAppSub[TrainAppSub.columns[vars_to_include]].shape[1]-3000, \
    TrainAppSub[['On_'+str(Appliance) for Appliance in range(1, NumAppliance+1)]].shape[1])

net = buildNetwork(TrainAppSub[TrainAppSub.columns[vars_to_include]].shape[1], \
    TrainAppSub[['On_'+str(Appliance) for Appliance in range(1, NumAppliance+1)]].shape[1])
trainer = BackpropTrainer(net, ds_train, batchlearning=True)
trainer.train()
